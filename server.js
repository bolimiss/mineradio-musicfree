...
// QR account login is intentionally not initialized in the MusicFree-only build.
const qishuiQrLogin = null;
const {
  getSpotifyConfig,
  clearSpotifyToken,
  saveSpotifyConfig,
...
const { MusicFreePluginHost, DEFAULT_MANIFEST_URL } = require('./musicfree-plugin-host');
const { createUniversalImport } = require('./universal-import');
...
const DISABLED_ACCOUNT_ROUTE_PREFIXES = [
  '/api/login/',
  '/api/qq/login/',
  '/api/kugou/login/',
  '/api/qishui/login/',
...
const DISABLED_ACCOUNT_ROUTES = new Set([
  '/api/login/status',
  '/api/spotify/config',
  '/api/qishui/status',
  '/api/qishui/logout',
...
const musicFreeHost = new MusicFreePluginHost({
  appRoot: __dirname,
  appVersion: APP_VERSION,
  defaultManifestUrl: process.env.MINERADIO_MUSICFREE_DEFAULT_MANIFEST || DEFAULT_MANIFEST_URL,
});
const universalImport = createUniversalImport({
  appRoot: __dirname,
  intervalMs: 5000,
  // Metadata parsing only. Playback URL, player and lyric APIs are deliberately
  // outside the Universal Import Layer.
  parse: (sourceUrl, platform) => musicFreeHost.importPlaylist(sourceUrl, platform),
});
universalImport.sync.start();
...
async function handleDiscoverHome() {
  ...
  if (!loggedIn) {
    let publicPlaylists = [];
    try {
      const publicResult = await personalized({ limit: 10, timestamp: Date.now() });
      const publicBody = publicResult && (publicResult.body || publicResult) || {};
      publicPlaylists = (publicBody.result || publicBody.data || [])
        .map(pl => mapDiscoverPlaylist(pl, '公开推荐歌单'))
        .filter(pl => pl.id && pl.name)
        .slice(0, 10);
    } catch (error) {
      console.warn('[DiscoverHome] public recommendations failed:', error.message);
    }
    return {
      loggedIn: false,
      user: null,
...
async function handleQQPublicRecommendations(limit, offset) {
  const num = Math.max(4, Math.min(20, Number(limit) || 12));
  const begin = Math.max(0, Number(offset) || 0);
  const json = await qqGetJSON('https://c.y.qq.com/v8/fcg-bin/fcg_v8_toplist_cp.fcg', {
    topid: 26,
    song_begin: begin,
    song_num: num,
    format: 'json',
  ...
  const rawSongs = Array.isArray(json && json.songlist) ? json.songlist : [];
  const songs = rawSongs.slice(0, num).map(entry => mapQQPlaylistTrack(entry && (entry.data || entry))).filter(song => song && song.name && (song.mid || song.id));
  return {
    provider: 'qq',
    source: 'qq-public-toplist',
    mode: 'public-toplist',
    provenance: 'qq-music-toplist-api',
    songs,
    message: songs.length ? '' : 'QQ 音乐热歌榜暂未返回歌曲。',
  };
}

function mapKugouPublicRankSong(item) {
  item = item || {};
  const filename = String(item.filename || item.songname || item.remark || '').trim();
  const split = filename.split(/\s+-\s+/);
  const artist = String(item.singername || (split.length > 1 ? split.shift() : '') || '').trim();
  const name = String(item.songname || item.remark || (split.length ? split.join(' - ') : filename) || '').trim();
  const hash = String(item.hash || item.filehash || '').trim();
  return {
    provider: 'kugou', source: 'kugou', type: 'song', id: hash || item.album_audio_id || item.audio_id || name,
    hash, fileHash: hash, albumAudioId: String(item.album_audio_id || item.audio_id || ''),
    albumId: String(item.album_id || ''), name, artist,
    cover: String(item.album_sizable_cover || item.img || '').replace('{size}', '240'),
    duration: (Number(item.duration) || 0) * 1000,
  };
}

async function fetchKugouPublicRankSongs(rankId, limit, page) {
  const endpoint = new URL('http://mobilecdn.kugou.com/api/v3/rank/song');
  endpoint.searchParams.set('rankid', String(rankId || 8888));
  endpoint.searchParams.set('page', String(Math.max(1, Number(page) || 1)));
  endpoint.searchParams.set('pagesize', String(Math.max(1, Math.min(50, Number(limit) || 20))));
  endpoint.searchParams.set('version', '9108');
  const response = await fetch(endpoint, { headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' }, signal: AbortSignal.timeout(12000) });
  if (!response.ok) throw new Error('KUGOU_PUBLIC_RANK_HTTP_' + response.status);
  const json = await response.json();
  const rows = json && json.data && Array.isArray(json.data.info) ? json.data.info : [];
  return rows.map(mapKugouPublicRankSong).filter(song => song.name).slice(0, limit);
}

async function fetchKugouPublicRanks(limit, offset) {
  const endpoint = 'http://mobilecdn.kugou.com/api/v3/rank/list?version=9108&plat=0&withsong=1';
  const response = await fetch(endpoint, { headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'application/json' }, signal: AbortSignal.timeout(12000) });
  if (!response.ok) throw new Error('KUGOU_PUBLIC_RANK_LIST_HTTP_' + response.status);
  const json = await response.json();
  const rows = json && json.data && Array.isArray(json.data.info) ? json.data.info : [];
  return rows.filter(item => item && item.rankid && item.rankname).slice(Math.max(0, Number(offset) || 0), Math.max(0, Number(offset) || 0) + limit).map(item => ({
    provider: 'kugou', source: 'kugou', type: 'playlist', id: 'rank:' + item.rankid,
    name: item.rankname, cover: String(item.img_cover || item.img_9 || item.bannerurl || '').replace('{size}', '300'),
    trackCount: Number(item.songnum || item.total || 0) || 0, playCount: Number(item.play_times || 0) || 0,
    creator: '酷狗音乐', tag: item.update_frequency || '公开榜单',
  }));
}

async function qqPublicPlaylists(limit, offset) {
  const charts = [
    [26, '热歌榜'], [27, '新歌榜'], [4, '流行指数榜'], [62, '飙升榜'],
    [5, '内地榜'], [6, '港台榜'], [3, '欧美榜'], [16, '韩国榜'],
    [17, '日本榜'], [28, '网络歌曲榜'], [29, '影视金曲榜'], [52, '腾讯音乐人原创榜'],
  ];
  const start = Math.max(0, Number(offset) || 0) % charts.length;
  const take = Math.max(1, Math.min(charts.length, Number(limit) || 12));
  const selected = Array.from({ length: take }, (_, index) => charts[(start + index) % charts.length]);
  return Promise.all(selected.map(async ([topid, name]) => {
    let cover = '';
    let trackCount = 100;
    try {
      const json = await qqGetJSON('https://c.y.qq.com/v8/fcg-bin/fcg_v8_toplist_cp.fcg', {
        topid, song_begin: 0, song_num: 1, format: 'json', platform: 'yqq.json', needNewCode: 0,
      }, { headers: { Referer: 'https://y.qq.com/n/ryqq/toplist/' + topid } });
      cover = json && json.topinfo && (json.topinfo.pic_v12 || json.topinfo.pic_h5 || json.topinfo.pic) || '';
      trackCount = Number(json && (json.total_song_num || json.cur_song_num)) || trackCount;
    } catch (_) {}
    return { provider: 'qq', source: 'qq', type: 'playlist', id: 'top:' + topid, name: 'QQ 音乐' + name, cover, trackCount, creator: 'QQ 音乐', tag: '公开榜单' };
  }));
}

async function qqPublicPlaylistTracks(id, limit) {
  const num = Math.max(1, Math.min(100, Number(limit) || 50));
  const chartMatch = String(id || '').match(/^top:(\d+)$/);
  if (chartMatch) {
    const result = await qqGetJSON('https://c.y.qq.com/v8/fcg-bin/fcg_v8_toplist_cp.fcg', {
      topid: chartMatch[1], song_begin: 0, song_num: num, format: 'json', inCharset: 'utf8', outCharset: 'utf-8', notice: 0, platform: 'yqq.json', needNewCode: 0,
    }, { headers: { Referer: 'https://y.qq.com/n/ryqq/toplist/' + chartMatch[1] } });
    const rows = Array.isArray(result && result.songlist) ? result.songlist : [];
    return rows.slice(0, num).map(entry => mapQQPlaylistTrack(entry && (entry.data || entry))).filter(song => song && song.name && (song.mid || song.id));
  }
  const result = await qqGetJSON('https://c.y.qq.com/qzone/fcg-bin/fcg_ucc_getcdinfo_byids_cp.fcg', {
    type: 1, utf8: 1, disstid: String(id || ''), song_begin: 0, song_num: num,
    loginUin: 0, hostUin: 0, format: 'json', inCharset: 'utf8', outCharset: 'utf-8',
    notice: 0, platform: 'yqq.json', needNewCode: 0,
  }, { cookie: false, headers: { Referer: 'https://y.qq.com/n/ryqq/playlist/' + encodeURIComponent(String(id || '')) } });
  const detail = result && result.cdlist && result.cdlist[0] ? result.cdlist[0] : {};
  const rows = Array.isArray(detail.songlist) ? detail.songlist : [];
  return rows.slice(0, num).map(mapQQPlaylistTrack).filter(song => song && song.name && (song.mid || song.id));
}

const SPOTIFY_PUBLIC_EDITORIAL = [
...
async function fetchSpotifyPublicPlaylist(id) {
  const html = await requestText('https://open.spotify.com/embed/playlist/' + encodeURIComponent(id), {
    headers: { 'User-Agent': 'Mozilla/5.0', Accept: 'text/html' }, timeoutMs: 22000,
  });
  const match = html.match(/<script id="__NEXT_DATA__" type="application\/json">([\s\S]*?)<\/script>/i);
  if (!match) throw new Error('SPOTIFY_PUBLIC_EMBED_DATA_MISSING');
  const parsed = JSON.parse(match[1]);
  return parsed && parsed.props && parsed.props.pageProps && parsed.props.pageProps.state && parsed.props.pageProps.state.data && parsed.props.pageProps.state.data.entity || null;
}

function mapSpotifyPublicTracks(entity, limit) {
  const rows = entity && Array.isArray(entity.trackList) ? entity.trackList : [];
  return rows.slice(0, limit).map(item => ({
    provider: 'spotify', source: 'spotify', type: 'song', id: String(item.uri || '').split(':').pop() || item.uid || '',
    uri: item.uri || '', name: item.title || '', artist: item.subtitle || '', duration: Number(item.duration || 0) || 0,
    cover: entity && entity.coverArt && entity.coverArt.sources && entity.coverArt.sources[0] && entity.coverArt.sources[0].url || '',
  })).filter(item => item.id && item.name);
}

async function handlePublicPlatformSongs(provider, limit, page) {
  const num = Math.max(4, Math.min(30, Number(limit) || 12));
  const pageIndex = Math.max(0, Number(page) || 0);
  const offset = pageIndex * num;
  if (provider === 'qq') return (await handleQQPublicRecommendations(num, offset)).songs || [];
  if (provider === 'kugou') return fetchKugouPublicRankSongs(8888, num, pageIndex + 1);
  if (provider === 'qishui') return (await handleQishuiSearch('热歌', num, '', offset)).songs || [];
  if (provider === 'spotify') return mapSpotifyPublicTracks(await fetchSpotifyPublicPlaylist(SPOTIFY_PUBLIC_EDITORIAL[0].id), num);
  const pageData = await fetchNeteasePlaylistTracksPage('3778678', num, offset);
  return (pageData.rawTracks || []).map(mapSongRecord).filter(song => song.id && song.name).slice(0, num);
}

async function handlePublicPlatformPlaylists(provider, limit, page) {
  const num = Math.max(1, Math.min(20, Number(limit) || 12));
  const pageIndex = Math.max(0, Number(page) || 0);
  const offset = pageIndex * num;
  if (provider === 'qq') return qqPublicPlaylists(num, offset);
  if (provider === 'kugou') return fetchKugouPublicRanks(num, offset);
  if (provider === 'qishui') {
    const songs = await handlePublicPlatformSongs('qishui', 20, pageIndex);
    return [{ provider: 'qishui', source: 'qishui', type: 'playlist', id: 'public-hot', name: '汽水热歌精选', cover: songs[0] && songs[0].cover || '', trackCount: songs.length, creator: '汽水音乐', tag: '公开曲库精选' }];
  }
  if (provider === 'spotify') {
    const cards = await Promise.all(SPOTIFY_PUBLIC_EDITORIAL.slice(0, num).map(async seed => {
      try {
        const entity = await fetchSpotifyPublicPlaylist(seed.id);
        return { provider: 'spotify', source: 'spotify', type: 'playlist', id: seed.id, name: entity && (entity.name || entity.title) || seed.name, cover: entity && entity.coverArt && entity.coverArt.sources && entity.coverArt.sources[0] && entity.coverArt.sources[0].url || '', trackCount: entity && entity.trackList && entity.trackList.length || 0, creator: 'Spotify', tag: '官方编辑歌单' };
      } catch (_) { return { provider: 'spotify', source: 'spotify', type: 'playlist', id: seed.id, name: seed.name, cover: '', trackCount: 0, creator: 'Spotify', tag: '官方编辑歌单' }; }
    }));
    return cards;
  }
  const result = await personalized({ limit: Math.min(50, offset + num), timestamp: Date.now() });
  const body = result && (result.body || result) || {};
  return (body.result || body.data || []).map(item => mapDiscoverPlaylist(item, '公开推荐歌单')).filter(item => item.id && item.name).slice(offset, offset + num);
}

async function handlePublicPlatformPlaylistTracks(provider, id, limit) {
  const num = Math.max(1, Math.min(100, Number(limit) || 50));
  if (provider === 'qq') return qqPublicPlaylistTracks(id, num);
  if (provider === 'kugou') return fetchKugouPublicRankSongs(String(id || '').replace(/^rank:/, '') || 8888, num);
  if (provider === 'qishui') return handlePublicPlatformSongs('qishui', num);
  if (provider === 'spotify') return mapSpotifyPublicTracks(await fetchSpotifyPublicPlaylist(id), num);
  const page = await fetchNeteasePlaylistTracksPage(id, num, 0);
  return (page.rawTracks || []).map(mapSongRecord).filter(song => song.id && song.name);
}
...
const server = http.createServer(async (req, res) => {
  ...
  // ---------- Universal Music Import Layer (metadata only) ----------
  if (pn === '/api/universal-import/import' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      const result = await universalImport.service.import(body.text || body.url || body.content || '', {
        receiver: String(body.receiver || req.headers['x-mineradio-receiver'] || 'web'),
      });
      sendJSON(res, { ok: true, ...result, tracks: result.playlist.tracks });
    } catch (error) {
      sendJSON(res, { ok: false, code: error.code || 'UNIVERSAL_IMPORT_FAILED', error: error.message || '链接导入失败' }, 400);
    }
    return;
  }

  if (pn === '/api/universal-import/preview' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await universalImport.service.preview(body.text || body.url || body.content || '', { receiver: String(body.receiver || 'web') }));
    } catch (error) {
      sendJSON(res, { ok: false, code: error.code || 'UNIVERSAL_IMPORT_PREVIEW_FAILED', error: error.message || '链接读取失败' }, 400);
    }
    return;
  }

  if (pn === '/api/universal-import/migrate' && req.method === 'POST') {
    const body = await readRequestBody(req);
    const playlists = universalImport.service.migrate(body.playlists);
    sendJSON(res, { ok: true, migrated: playlists.length, playlists });
    return;
  }

  if (pn === '/api/universal-import/playlists' && req.method === 'GET') {
    sendJSON(res, { ok: true, playlists: universalImport.database.list(), sync: universalImport.sync.status() });
    return;
  }

  if (pn === '/api/universal-import/sync-all' && req.method === 'POST') {
    try { sendJSON(res, { ok: true, ...(await universalImport.sync.syncAll('manual')) }); }
    catch (error) { sendJSON(res, { ok: false, error: error.message || 'SYNC_FAILED' }, 500); }
    return;
  }

  const universalSyncMatch = pn.match(/^\/api\/universal-import\/playlists\/([^/]+)\/sync$/);
  if (universalSyncMatch && req.method === 'POST') {
    try { sendJSON(res, { ok: true, ...(await universalImport.sync.syncOne(decodeURIComponent(universalSyncMatch[1]), 'manual')) }); }
    catch (error) { sendJSON(res, { ok: false, error: error.message || 'SYNC_FAILED' }, error.message === 'PLAYLIST_NOT_FOUND' ? 404 : 500); }
    return;
  }

  const universalPlaylistMatch = pn.match(/^\/api\/universal-import\/playlists\/([^/]+)$/);
  if (universalPlaylistMatch && req.method === 'DELETE') {
    const removed = universalImport.database.remove(decodeURIComponent(universalPlaylistMatch[1]));
    sendJSON(res, { ok: removed }, removed ? 200 : 404);
    return;
  }

  if (pn === '/api/universal-import/test-center' && req.method === 'GET') {
    sendJSON(res, { ok: true, diagnostics: universalImport.database.diagnostics(), sync: universalImport.sync.status(), events: universalImport.database.state.events.slice(0, 100) });
    return;
  }

  // ---------- MusicFree 插件音源 ----------
  if (pn === '/api/musicfree/plugins' && req.method === 'GET') {
    await musicFreeHost.ready;
    sendJSON(res, { plugins: musicFreeHost.list(), defaultManifestUrl: musicFreeHost.defaultManifestUrl });
    return;
  }

  if (pn === '/api/musicfree/search' && req.method === 'GET') {
    try {
      ...
      const pluginId = String(url.searchParams.get('plugin') || '').trim();
      if (!keywords) { sendJSON(res, { songs: [], hasMore: false, offset, limit, nextOffset: offset }); return; }
      const result = await musicFreeHost.search(keywords, page, pluginId);
      ...
      sendJSON(res, {
        ...
        hasMore: !!result.hasMore && songs.length > 0,
      });
    } catch (error) {
      sendJSON(res, { songs: [], error: error.message || 'MUSICFREE_SEARCH_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/musicfree/import-network' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      const result = await musicFreeHost.importNetwork(body.url);
      sendJSON(res, { ok: true, ...result });
    } catch (error) {
      sendJSON(res, { ok: false, error: error.message || 'MUSICFREE_IMPORT_FAILED' }, 400);
    }
    return;
  }

  if (pn === '/api/musicfree/import-local' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      const result = await musicFreeHost.importLocal(body.source, body.filename, body.zipBase64 || '');
      sendJSON(res, { ok: true, ...result });
    } catch (error) {
      sendJSON(res, { ok: false, error: error.message || 'MUSICFREE_LOCAL_IMPORT_FAILED' }, 400);
    }
    return;
  }

  if (pn === '/api/musicfree/import-playlist' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      const result = await musicFreeHost.importPlaylist(body.url, body.provider || '');
      sendJSON(res, { ok: true, ...result });
    } catch (error) {
      sendJSON(res, {
        ok: false,
        code: error.code || 'MUSICFREE_PLAYLIST_IMPORT_FAILED',
        error: error.message || '歌单链接解析失败',
  ...
  if (pn === '/api/musicfree/reload' && req.method === 'POST') {
    try { sendJSON(res, { ok: true, ...(await musicFreeHost.reload()) }); }
    catch (error) { sendJSON(res, { ok: false, error: error.message || 'MUSICFREE_RELOAD_FAILED' }, 500); }
    return;
  }

  if (pn === '/api/musicfree/group-action' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      const group = { manifestUrl: body.manifestUrl || '', pluginId: body.pluginId || '' };
      if (body.action === 'remove') {
        sendJSON(res, await musicFreeHost.removeGroup(group));
      } else if (body.action === 'enable' || body.action === 'disable') {
        sendJSON(res, await musicFreeHost.setGroupEnabled(group, body.action === 'enable'));
      } else {
        sendJSON(res, { ok: false, error: 'INVALID_GROUP_ACTION' }, 400);
      }
    } catch (error) {
      sendJSON(res, { ok: false, error: error.message || 'MUSICFREE_GROUP_ACTION_FAILED' }, 500);
    }
    return;
  }

  if (pn.startsWith('/api/musicfree/plugins/') && req.method === 'DELETE') {
    const id = decodeURIComponent(pn.slice('/api/musicfree/plugins/'.length));
    const removed = musicFreeHost.remove(id);
    sendJSON(res, { ok: removed, error: removed ? '' : 'BUILTIN_SOURCE_CANNOT_BE_REMOVED', plugins: musicFreeHost.list() }, removed ? 200 : 400);
    return;
  }

  if (pn.startsWith('/api/musicfree/plugins/') && pn.endsWith('/config') && req.method === 'POST') {
    try {
      const id = decodeURIComponent(pn.slice('/api/musicfree/plugins/'.length, -'/config'.length));
      const body = await readRequestBody(req);
      sendJSON(res, await musicFreeHost.setPluginVariables(id, body.values || body.variables || {}));
    } catch (error) {
      sendJSON(res, { ok: false, error: error.message || 'MUSICFREE_CONFIG_FAILED' }, 400);
    }
    return;
  }

  if (pn === '/api/musicfree/media' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await musicFreeHost.resolveMedia(body.song || {}, body.quality));
    } catch (error) {
      sendJSON(res, { provider: 'musicfree', url: '', error: error.message || 'MUSICFREE_MEDIA_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/musicfree/bilibili-audio' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await musicFreeHost.resolveBilibiliAudio(body.song || {}, body.quality || 'original'));
    } catch (error) {
      sendJSON(res, {
        provider: 'musicfree',
        source: 'bilibili',
        ...
        attemptedSources: 1,
        url: '',
        error: error.message || 'BILIBILI_AUDIO_FAILED',
      }, 500);
    }
    return;
  }

  if (pn === '/api/musicfree/video' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await musicFreeHost.resolveVideo(body.song || {}, body.quality || 'auto'));
    } catch (error) {
      sendJSON(res, { provider: 'musicfree-video', mediaType: 'video', error: error.message || 'MUSICFREE_VIDEO_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/musicfree/lyric' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await musicFreeHost.lyric(body.song || {}));
    } catch (error) {
      sendJSON(res, { lyric: '', tlyric: '', error: error.message || 'MUSICFREE_LYRIC_FAILED' }, 500);
    }
    return;
  }

  if (pn === '/api/musicfree/diagnose' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      sendJSON(res, await musicFreeHost.diagnose(body.pluginId || body.id || '', body.query || '稻香'));
    } catch (error) {
      sendJSON(res, { ok: false, error: error.message || 'MUSICFREE_DIAGNOSE_FAILED', details: error.details || null }, 400);
    }
    return;
  }

  if (pn === '/api/musicfree/audio' && req.method === 'GET') {
    try { await musicFreeHost.proxyAudio(req, res, url.searchParams.get('token')); }
    catch (error) {
  ...
  if (pn === '/api/musicfree/video-stream' && req.method === 'GET') {
    try { await musicFreeHost.proxyVideo(req, res, url.searchParams.get('token')); }
    catch (error) {
      if (!res.headersSent) res.writeHead(502, { 'Cache-Control': 'no-store' });
      res.end();
    }
  ...
  if (DISABLED_ACCOUNT_ROUTES.has(pn) || DISABLED_ACCOUNT_ROUTE_PREFIXES.some(prefix => pn.startsWith(prefix))) {
    sendJSON(res, {
      ok: false,
      error: 'ACCOUNT_LOGIN_REMOVED',
      message: '当前版本已移除网络账号入口，请使用 MusicFree 音源。',
    }, 410);
    return;
  }
  ...
  if (pn === '/api/platform/recommendations') {
    const provider = /^(netease|qq|kugou|qishui|spotify)$/.test(String(url.searchParams.get('provider') || ''))
      ? String(url.searchParams.get('provider')) : 'netease';
    const type = url.searchParams.get('type') === 'playlists' ? 'playlists' : 'songs';
    const limit = Math.max(1, Math.min(30, parseInt(url.searchParams.get('limit') || '12', 10) || 12));
    const page = Math.max(0, parseInt(url.searchParams.get('page') || '0', 10) || 0);
    try {
      const items = type === 'playlists'
        ? await handlePublicPlatformPlaylists(provider, limit, page)
        : await handlePublicPlatformSongs(provider, limit, page);
      sendJSON(res, { provider, type, page, source: provider + '-public', items, songs: type === 'songs' ? items : [], playlists: type === 'playlists' ? items : [], message: items.length ? '' : '平台公开接口暂未返回内容。' });
    } catch (err) {
      console.error('[PlatformRecommendations]', provider, type, err);
      sendJSON(res, { provider, type, source: provider + '-public', items: [], songs: [], playlists: [], error: err.message, message: '平台公开接口读取失败，请稍后刷新。' }, 500);
    }
    return;
  }

  if (pn === '/api/platform/playlist/tracks') {
    const provider = /^(netease|qq|kugou|qishui|spotify)$/.test(String(url.searchParams.get('provider') || ''))
      ? String(url.searchParams.get('provider')) : 'netease';
    ...
    try {
      ...
      const tracks = await handlePublicPlatformPlaylistTracks(provider, id, limit);
      sendJSON(res, { provider, id, tracks, total: tracks.length, hasMore: false });
    } catch (err) {
      console.error('[PlatformPlaylistTracks]', provider, id, err);
      sendJSON(res, { provider, id, tracks: [], error: err.message }, 500);
    }
    return;
  }
  ...
  if (pn === '/api/qq/recommendations') {
    try {
      ...
      sendJSON(res, await handleQQPublicRecommendations(limit));
    } catch (err) {
      console.error('[QQRecommendations]', err);
      sendJSON(res, { provider: 'qq', source: 'qq-public-toplist', error: err.message, songs: [] }, 500);
    }
    return;
  }
